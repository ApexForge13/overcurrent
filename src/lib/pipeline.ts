...
export async function runVerifyPipeline(
  ...
  // Fetch ALL triaged sources — no cap. Fetching is free (HTTP only, no AI).
  // Articles are already truncated to 3,000 words and split across 6 regional
  // debates, so even 30+ sources per region fits within model context limits.
  const topSources = triageResult.sources
  console.log(`[fetch] Fetching all ${topSources.length} triaged sources`)
  ...
  const story = await prisma.$transaction(async (tx: Omit<PrismaClient, '$connect' | '$disconnect' | '$on' | '$transaction' | '$use' | '$extends'>) => {
    const story = await tx.story.create({
      data: {
        ...
        lobbyData: JSON.stringify({
          pattern: synthesisResult.thePattern || '',
          summary: synthesisResult.synopsis || '',
        }),
        briefingData: JSON.stringify(synthesisResult.briefing || {}),
        vaultData: JSON.stringify({
          claims: synthesisResult.claims || [],
          framing_split: synthesisResult.framingSplit || [],
          discrepancies: synthesisResult.discrepancies || [],
          omissions: synthesisResult.omissions || [],
          buried_findings: synthesisResult.buriedEvidence || [],
        }),
