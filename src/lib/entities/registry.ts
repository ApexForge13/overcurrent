/**
 * Orchestrator — pulls from all four sources, dedupes, upserts.
 *
 ...
 *   4. ETFs (LAST — overrides SEC's 'equity' default with 'etf' for SPY/QQQ/etc.)
 *
 * The orchestrator tolerates partial source failure — if CoinGecko is down
 * the seed still lands SEC + futures + ETFs, with a warning logged. This
 * is intentional: a one-off seed shouldn't bail because a third-party API
 * hiccupped mid-run.
 */
...
export async function loadEntityRegistry(
  ...
  const bySource: Record<string, number> = { sec: 0, coingecko: 0, futures: 0, etfs: 0 }
