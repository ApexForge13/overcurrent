...
describe('queue names contract', () => {
  it('exposes exactly 9 queues (3 gap-score + 2 phase-1c trigger infra + 4 paper-trading)', () => {
  ...
  it('every value is lowercase kebab-case with no colon (BullMQ constraint)', () => {
    ...
    const pattern = /^(gap-score|paper-trading|candidate-generator|trigger-scan)(-[a-z-]+)?$/
  ...
  it('uses the expected domain prefixes (gap-score, paper-trading, candidate-generator, trigger-scan)', () => {
    const prefixes = new Set(
      Object.values(QUEUE_NAMES).map((v) => {
        ...
        return v // candidate-generator and trigger-scan are their own prefixes
      }),
    )
    expect(prefixes).toEqual(
      new Set(['gap-score', 'paper-trading', 'candidate-generator', 'trigger-scan']),
    )
  })
